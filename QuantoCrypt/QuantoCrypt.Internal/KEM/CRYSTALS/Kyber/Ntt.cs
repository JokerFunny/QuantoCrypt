namespace QuantoCrypt.Internal.KEM.CRYSTALS.Kyber
{
    ...
    internal class Ntt
    {
        internal static readonly short[] srZetas = {
        ...
        internal static void NTT(short[] r)
        {
            ...
            for (int len = 128; len >= 2; len >>= 1)
            {
                ...
                {
                    short zeta = srZetas[k++];
                    for (j = start; j < start + len; ++j)
                    {
                        short t = _FactorQMulMont(zeta, r[j + len]);
                        r[j + len] = (short)(r[j] - t);
                        r[j] = (short)(r[j] + t);
                    }
                }
            ...
            for (int len = 2; len <= 128; len <<= 1)
            {
                for (int start = 0; start < 256; start = j + len)
                {
                    ...
                    for (j = start; j < start + len; ++j)
                    {
                        ...
                        r[j + len] = _FactorQMulMont(zeta, r[j + len]);
        ...
        {
            short outVal0 = _FactorQMulMont(a1, b1);
            outVal0 = _FactorQMulMont(outVal0, zeta);
            outVal0 += _FactorQMulMont(a0, b0);
            r[off] = outVal0;

            short outVal1 = _FactorQMulMont(a0, b1);
            outVal1 += _FactorQMulMont(a1, b0);
            r[off + 1] = outVal1;
        }

        private static short _FactorQMulMont(short a, short b)
            => Reduce.MontgomeryReduce(a * b);
    }
}
