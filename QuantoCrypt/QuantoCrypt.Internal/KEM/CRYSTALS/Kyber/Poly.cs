namespace QuantoCrypt.Internal.KEM.CRYSTALS.Kyber
{
    ...
    internal class Poly
    {
        private readonly short[] _rCoeffs = new short[KyberEngine.N];
        ...
        {
            byte[] buf = new byte[_rKyberEngine.Eta1 * KyberEngine.N / 4];
            _rSymmetric.Prf(buf, seed, nonce);
            Cbd.Eta(this, buf, _rKyberEngine.Eta1);
        }
        ...
        internal void CompressPoly(byte[] r, int off)
        {
            ...
            if (_rKyberEngine.PolyCompressedBytes == 128)
            {
            ...
            else if (_rKyberEngine.PolyCompressedBytes == 160)
            {
                for (int i = 0; i < KyberEngine.N / 8; i++)
                {
                    for (int j = 0; j < 8; j++)
