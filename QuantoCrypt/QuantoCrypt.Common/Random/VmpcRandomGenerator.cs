using QuantoCrypt.Common.Utilities;

namespace QuantoCrypt.Common.Random
{
    /// <summary>
    /// Generator based on Variably Modified Permutation Composition.
    /// </summary>
    public sealed class VmpcRandomGenerator : IRandomGenerator
    {
        /// <remarks>
        /// Permutation generated by code:
        /// <code>
        /// // First 1850 fractional digit of Pi number. 
        /// byte[] key = new BigInteger("14159265358979323846...5068006422512520511").ToByteArray();
        /// _s = 0;
        /// _rP = new byte[256];
        /// for (int i = 0; i &lt; 256; i++) 
        /// {
        ///     _rP[i] = (byte) i;
        /// }
        /// for (int m = 0; m &lt; 768; m++) 
        /// {
        ///     _s = _rP[(_s + _rP[m &amp; 0xff] + key[m % key.length]) &amp; 0xff];
        ///     byte temp = _rP[m &amp; 0xff];
        ///     _rP[m &amp; 0xff] = _rP[_s &amp; 0xff];
        ///     _rP[_s &amp; 0xff] = temp;
        /// } </code>
        /// </remarks>
        private readonly byte[] _rP =
        {
            ,
        };

        /// <remarks>
        /// Value generated in the same way as <c>_rP</c>.
        /// </remarks>
        private byte _s = 0xbe;
        private byte _n = 0;

        /// <summary>
        /// Default ctor.
        /// </summary>
        public VmpcRandomGenerator()
        { }

        public void AddSeedMaterial(byte[] seed)
        {
            if (seed == null)
                return;

            AddSeedMaterial(seed.AsSpan());
        }

        public void AddSeedMaterial(ReadOnlySpan<byte> seed)
        {
            for (int m = 0; m < seed.Length; m++)
            {
                byte pn = _rP[_n];
                _s = _rP[(_s + pn + seed[m]) & 0xff];
                _rP[_n] = _rP[_s];
                _rP[_s] = pn;
                _n = (byte)(_n + 1);
            }
        }

        public void AddSeedMaterial(long seed)
        {
            Span<byte> bytes = stackalloc byte[8];
            PackUtilities.UInt64_To_BE((ulong)seed, bytes);

            AddSeedMaterial(bytes);
        }

        public void NextBytes(byte[] bytes)
            => NextBytes(bytes, 0, bytes.Length);

        public void NextBytes(byte[] bytes, int start, int len)
            => NextBytes(bytes.AsSpan(start, len));

        public void NextBytes(Span<byte> bytes)
        {
            lock (_rP)
            {
                for (int i = 0; i < bytes.Length; ++i)
                {
                    byte pn = _rP[_n];
                    _s = _rP[(_s + pn) & 0xFF];
                    byte ps = _rP[_s];
                    bytes[i] = _rP[(_rP[ps] + 1) & 0xFF];
                    _rP[_s] = pn;
                    _rP[_n] = ps;
                    _n = (byte)(_n + 1);
                }
            }
        }
    }
}
