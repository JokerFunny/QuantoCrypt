...
namespace QuantoCrypt.Common.Digest
{
    ...
    public class Sha3Digest : KeccakDigest
    {
        ...
        public Sha3Digest(int bitLength)
            : base(_CheckBitLength(bitLength))
        { }
            ...
            byte[] output, int outputOffset, int outputLengthBits)
        {
            ...
            if (inputOffset > (input.Length - requiredInputLength))
                throw new ArgumentException("Input buffer too short");

            int requiredOutputLength = (outputLengthBits + 7) >> 3;
            if (outputOffset > (output.Length - requiredOutputLength))
            ...
            {
                NatUtilities.XorTo64(rate64, input, inputOffset, state, 0);
                inputOffset += rate64;
            ...
            NatUtilities.XorTo64(remaining64, input, inputOffset, state, 0);
            ...
            PackUtilities.UInt64_To_LE(state, 0, count64, output, outputOffset);

            if ((outputLengthBits & 32) != 0)
                BinaryPrimitives.WriteUInt32LittleEndian(output.AsSpan(outputOffset + (count64 << 3)), (uint)state[count64]);
        }
            ...
            int outputLengthBits)
        {
            ...
            while (inputLengthBits >= rate)
            {
                NatUtilities.XorTo64(rate64, input, state);
                input = input[rate64..];
                inputLengthBits -= rate;
            ...
            NatUtilities.XorTo64(remaining64, input, state);
            ...
            Debug.Assert(outputLengthBits <= rate);
            int count64 = outputLengthBits >> 6;
            PackUtilities.UInt64_To_LE(state[..count64], output);

            if ((outputLengthBits & 32) != 0)
                BinaryPrimitives.WriteUInt32LittleEndian(output[(count64 << 3)..], (uint)state[count64]);
        }
        ...
        private static int _CheckBitLength(int bitLength)
        {
            switch (bitLength)
            {
                case 224:
